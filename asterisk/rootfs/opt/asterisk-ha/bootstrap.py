...
if asterisk_conf.exists():
    ...
    def set_ast_option(src, key, value):
        ...
        match=re.search(r'(?im)^\s*\[options\]\s*$', src)
        if not match:
            return src.rstrip()+f'\n\n[options]\n{line}\n'
        pos=match.end()
        return src[:pos]+f'\n{line}'+src[pos:]
...
if modules.exists():
    text=modules.read_text(errors='ignore')
    clean_start='; ASTERISK HA OPTIONAL MODULE CLEANUP START'
    clean_end='; ASTERISK HA OPTIONAL MODULE CLEANUP END'
    text=re.sub(
        rf'(?ms)^\s*{re.escape(clean_start)}\s*$.*?^\s*{re.escape(clean_end)}\s*$\n?',
        '', text
    )
...
if cel_custom.exists():
    ...
    migrated=re.sub(r'(?i)\bCIDName\b', 'Name', cel_text)
    migrated=re.sub(r'(?i)\bCIDNum\b', 'Num', migrated)
    if migrated != cel_text:
        cel_custom.write_text(migrated)
        print('[CEL] migrated advanced fields CIDName/CIDNum -> Name/Num')
