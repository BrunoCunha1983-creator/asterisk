...
async function securityPage(a){
  ...
  let jailRows=(s.jail_status||[]).map(j=>`<div class=item><div class=status-head><b>${esc(j.name||'jail')}</b><span class="status-badge ${j.enabled===false?'disabled':s.running?'online':'offline'}">${j.enabled===false?'DESATIVADO':s.running?'ATIVO':'OFFLINE'}</span></div><div class=sub>Falhas atuais: ${esc(j.current_failed||0)} · Bans atuais: ${esc(j.currently_banned||0)} · Total bans: ${esc(j.total_banned||0)}</div>${j.banned_ips&&j.banned_ips.length?`<div class=sub>IPs: ${esc(j.banned_ips.join(', '))}</div>`:''}</div>`).join('');
  a.innerHTML=`<div class=grid><div class=card><div class=sub>Fail2ban</div><div class="big ${s.running?'ok':'bad'}">${s.running?'ATIVO':'OFFLINE'}</div><div class=sub>${esc(s.error||'')}</div></div><div class=card><div class=sub>Jails</div><div class=big>${(s.jails||[]).length}</div></div><div class=card><div class=sub>IPs banidos agora</div><div class=big>${s.currently_banned||0}</div></div><div class=card><div class=sub>Total de bans</div><div class=big>${s.total_banned||0}</div></div></div><div class=card><h2>Segurança / Fail2ban</h2><div class=note>Proteção automática para brute-force SIP/PJSIP e scans HTTP. Redes privadas/Ingress estão na whitelist por defeito para reduzir o risco de auto-bloqueio.</div>${jailRows||'<div class=item>Sem informação de jails. Verifica se o Fail2ban arrancou.</div>'}${s.banned_ips&&s.banned_ips.length?`<h3>IPs atualmente bloqueados</h3><pre>${esc(s.banned_ips.join('\n'))}</pre>`:''}<div class=actions><button class=btn onclick="securityPage(E('#app'))">Atualizar segurança</button></div></div>`;
}
'''
