    ...
    for ivr in ivrs:
        ...
        for option in ivr.get('options') or []:
            ...
            if option.get('type') == 'ivr' and _clean_id(option.get('value')) == ivr_id:
                # Self loops are allowed only intentionally via timeout/retry, not a key mapping.
...
def _append_commands(lines, commands):
    for index, command in enumerate(commands):
        prefix = ' same => n,' if index or lines else ' same => n,'
        lines.append(prefix + command)


def render_ivrs(conf, data):
    """Append IVR entry extensions and write dedicated IVR contexts."""
    data, _ = ensure_ivr_state(data)
    validate_ivrs(data)
    ivrs = [i for i in (data.get('ivrs') or []) if i.get('enabled', True)]

    # Entry extensions live in from-internal through extensions_gui.conf.
    entry = conf / 'extensions_gui.conf'
    if ivrs:
        with entry.open('a') as f:
            f.write('\n; IVR ENTRY EXTENSIONS\n')
            for ivr in ivrs:
                ivr_id = _clean_id(ivr.get('id'))
                extension = _number(ivr.get('extension'))
    ...
    for ivr in ivrs:
        ...
        out += [
            'exten => i,1,Set(IVR_ATTEMPT=$[${IVR_ATTEMPT}+1])',
        ]
        if invalid_prompt:
...
function ivrs(a){
  ...
  <div class=note>Cada IVR recebe uma extensão interna própria. Exemplo: cria o IVR Principal na extensão <b>600</b> e coloca <b>600</b> como destino de entrada no HT503 ou GSM. O campo Prompt é o nome de um som Asterisk sem extensão, por exemplo <code>custom/ivr-main</code>.</div>
  ${x.map((v,i)=>`<div class=item><h3>${esc(v.name||v.id||'IVR')}</h3><div class=row>
  ...
  <div class=actions><button class=btn onclick=addIVR()>+ IVR</button><button class="btn primary" onclick=saveIVRs()>Guardar e aplicar</button></div></div>`;
}
...
async function saveIVRs(){captureIVRs();await savePbx()}
'''
